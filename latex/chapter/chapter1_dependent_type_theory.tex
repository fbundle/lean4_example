...
\note{
	I don't understand the internal working of types, but this is my theory on how to make senses of different level sum/product/hom types. \hl{2} is at level 0 and \hl{Nat} is at level 1, one can lift \hl{2} into level 1 into a new type \hl{Two} which has only one term \hl{2}, then we can define \hl{2 $\times$ Nat} by \hl{Two $\times$ Nat}. Similarly, one can lift \hl{Nat} into level 2 and lift every natural number into level 1.
}
