...
Let \hl{p: Prop} and \hl{q: Prop} be propositions and \hl{f: p $\to$ q} be a map. If \hl{hp: p} is a proof, then \hl{f hp: q} is also a proof. This is known as \textit{modus ponens} or implication elimination rule. In lean, \hl{p $\to$ q: Prop} is a proposition

\begin{itemize}
	\item \hl{Implies} admits one introduction rule by declaring a function of type \hl{p $\to$ q}
	
	\item \hl{Implies} admits one implication elimination rule by function application as described above
\end{itemize}
